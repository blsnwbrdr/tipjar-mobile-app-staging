...
export class CountryListing extends Component {
  ...
  _getLocationAsync = async () => {
    ...
    let latlng = {latitude:location.coords.latitude,longitude:location.coords.longitude}
    console.log(latlng);
    let address = await Location.reverseGeocodeAsync(latlng);
    let country = address[0].country;
    console.log(country);
    this.onPressTipData(country);
  };
